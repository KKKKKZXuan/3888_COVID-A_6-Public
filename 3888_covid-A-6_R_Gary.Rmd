...
data_sw_age_developed <- covid_data_age %>%
...
plot_sw_age_developed <- ggplot(data = data_sw_age_developed,
                                aes(x = Date,
...
plot_sw_age_developed

ggsave("Gary_age_developed.png",
       plot = plot_sw_age_developed,
       width = 2560,
       height = 1440,
       units = "px",
       dpi = "retina",
...
data_ro_age_developed <- covid_data_age %>%
  filter(Location == "Romania") %>%
  filter(Date <= "2021-12-31" & Date >= "2020-06-01") %>%
  filter(Age_group %in% group2) %>%
  mutate(Month = month(Date))

plot_ro_age_developed <- ggplot(data = data_ro_age_developed,
                                aes(x = Date,
                                    y = Group_deaths_number,
                                    group = Age_group,
                                    color = Age_group)) +
...
plot_ro_age_developed

ggsave("Gary_age_developing.png",
       plot = plot_ro_age_developed,
       width = 2560,
       height = 1440,
       units = "px",
       dpi = "retina",
